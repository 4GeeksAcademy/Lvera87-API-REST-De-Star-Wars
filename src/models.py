...
class User(db.Model):
    ...
    # Guardar hash de contraseña, no texto plano
    ...
    # Campo opcional para activar/desactivar usuarios
    ...
    favorite_people: Mapped[List["FavoritePerson"]] = relationship(
        "FavoritePerson", back_populates="user", cascade="all, delete-orphan")
    ...
    favorite_vehicles: Mapped[List["FavoriteVehicle"]] = relationship(
        "FavoriteVehicle", back_populates="user", cascade="all, delete-orphan")
    ...
    def serialize(self):
        ...
        return {
            ...
            # aquí se obtiene la lista de planetas favoritos
            "favorite_planet_ids": [fav.planet_id for fav in self.favorite_planets],
            "favorite_person_ids": [fav.person_id for fav in self.favorite_people],
            # NUEVO
            "favorite_vehicle_ids": [fav.vehicle_id for fav in self.favorite_vehicles]
        }
...
class Planet(db.Model):
    ...
    residents: Mapped[List["Person"]] = relationship(
        "Person", back_populates="homeworld")
...
class Person(db.Model):
    ...
    homeworld_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("planets.id"), nullable=True)

    # --- Relaciones ---
    homeworld: Mapped["Planet"] = relationship(
        "Planet", back_populates="residents")
    favorited_by: Mapped[List["FavoritePerson"]] = relationship(
        "FavoritePerson", back_populates="person", cascade="all, delete-orphan")
...
class Vehicle(db.Model):
    ...
    favorited_by: Mapped[List["FavoriteVehicle"]] = relationship(
        "FavoriteVehicle", back_populates="vehicle", cascade="all, delete-orphan")
...
class FavoritePlanet(db.Model):
    ...
    planet_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("planets.id"), primary_key=True)
    ...
    def __repr__(self):
        # este es el resprentacion de usada en los print en la terminal
...
class FavoritePerson(db.Model):
    ...
    user: Mapped["User"] = relationship(
        "User", back_populates="favorite_people")
    person: Mapped["Person"] = relationship(
        "Person", back_populates="favorited_by")
...
class FavoriteVehicle(db.Model):
    ...
    vehicle_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("vehicles.id"), primary_key=True)

    # --- Relaciones ---
    user: Mapped["User"] = relationship(
        "User", back_populates="favorite_vehicles")
    vehicle: Mapped["Vehicle"] = relationship(
        "Vehicle", back_populates="favorited_by")
